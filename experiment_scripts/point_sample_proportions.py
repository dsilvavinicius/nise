...
if __name__ == '__main__':
    ...
    PROPORTIONS = [[0.1, 0.1], [0.4, 0.4], [0.25, 0.25]]
    for P in PROPORTIONS:
        print(f"=============== p_on_surf={P[0]}, p_off_surf={P[1]}, p_time={1.0-P[0]-P[1]} ===============")
        EXPERIMENT = f"{MESH}_meancurvature_{EPOCHS}epochs_sitz_init_p{P[0]},{P[1]},{1.0-P[0]-P[1]}"
        ...
        model.reset_weights()
        dataset.time_sampler = torch.distributions.uniform.Uniform(-1., 1.)
        dataset.fraction_on_surface = P[0]
        dataset.fraction_off_surface = P[1]
        ...
        n_on_surface = math.ceil(BATCHSIZE * P[0])
        n_off_surface = math.floor(BATCHSIZE * P[1])
